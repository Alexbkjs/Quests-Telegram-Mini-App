...
const ButtonGradient: React.FC<ButtonGradientProps> = ({
  ...
  return (
    <button
      className={`btn btn-primary flex-1 max-w-52 bg-gradient-to-r ${gradientColors} ${textColor} ${size} text-xl rounded-3xl font-bold shadow-md hover:shadow-lg transition-all duration-300 ${className}`}
      onClick={onClick}
    >
      {text}
    </button>
